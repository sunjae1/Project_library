...
public class TimeRepository {
    ...
    public List<String> getReservedTimes(String date, Room room) {
        return reservedTimes.getOrDefault(generateKey(date, room), new ArrayList<>());//getOrDefault : 키 있으면 값 반환, 없으면 new ArrayList<>() 기본값 반환.
    }

    public boolean isTimeAvailable(String date, Room room, String time) {
        return !getReservedTimes(date, room).contains(time);
    }

    private String generateKey(String date, Room room) {
